...
sample_n = size(train_set,1);   %设置样本数目
samples_weight = ones(sample_n,1)/sample_n; %将权重初始化为1/n

for turn=1:no_of_hypothesis
	adaboost_model.parameters{turn} = threshold_tr(train_set,samples_weight,labels); %得到参数
	[L,hits,error_rate] = threshold_te(adaboost_model.parameters{turn},... %测试
	...
	samples_weight(t_labeled) = samples_weight(t_labeled)*...
					((error_rate)/(1-error_rate));				%更新权重

	% Normalization
	samples_weight = samples_weight/sum(samples_weight);    %归一化处理
end
