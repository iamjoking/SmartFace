function [L,hits,ferr,perr] = ADABOOST_te(adaboost_model,test_set,...
                                true_labels)
%
% ADABOOST TESTING
%
...
%                    perr: the postive instance is classifed as negative
%                    instance
%                    ferr: the negtive instance is classifed as positive
%                    instance
%
...
for i=1:hypothesis_n
	[temp_L(:,:,i),hits,error_rate] = threshold_te(adaboost_model.parameters{i},...
...
predict_classes = likelihood2class(L);
ferr = 0;
perr = 0;
for i = 1 : length(predict_classes)
    if predict_classes(i) ~= true_labels(i) && true_labels(i) == 1
        ferr = ferr + 1;
    end
    if predict_classes(i) ~= true_labels(i) && true_labels(i) == 2
        perr = perr + 1;
    end
end
